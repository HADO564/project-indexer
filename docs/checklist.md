...
- [ ] **Plugins** — two shapes: a UI plugin over data the backend already produces (purpose-built panels, per-kind actions, non-tracker views), and a UI plugin paired with a Rust `Detector` for anything the backend cannot yet see. The generic renderer means the first shape is an improvement rather than a gap. Blocked on the trust design, not on the rendering: a CSP is now in place, but capability scoping and a host API that replaces raw `invoke` are not. A runtime loader for native plugins is declined outright — see `ROADMAP.md` → Plugins → Trust.
