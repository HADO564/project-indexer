...
- `models/` — `Project` (`trackers: Vec<Tracker>`), `UpdateProject` (partial-update DTO with a double-`Option` deserializer to distinguish "omit field" from "set to null"), `Tracker` (enum: `Git(GitInfo)` / `Unreal(UnrealInfo)` carry real detected data; `Unity`/`Blender` are still bare units with no detector), `GitInfo`, `UnrealInfo`, `InstalledApp`.
...
- `detectors/` — the `Detector` trait (`detector.rs`) plus a `DetectorRunner` (`runner.rs`) that runs every registered detector against a path and collects a `Tracker` from each one that recognizes it. `DetectorRunner::default()` registers `Gitector` and `UnrealDetector`.
- `errors/` — `ProjectError` (surfaced to the frontend as a plain string), `GitError`, `UnrealError`, `DetectorError` (wraps `Io`/`Git`/`Unreal`, converted to `ProjectError::Detection` at the command boundary).
...
1. `create_project` runs `detect_project` (every registered detector) best-effort against the new directory — a detection failure is logged, not surfaced, since the project is still worth tracking.
2. `refresh_project_trackers` re-runs it for an existing project and _does_ surface a failure — an explicit, user-triggered retry.
3. `detect_project_trackers` runs detection against a directory that isn't a project yet — nothing is read from or written to the store. This backs the browse-autocomplete flow below.
4. Two real detectors are registered:
   ...
   - `Unity` and `Blender` remain bare `Tracker` variants with no detector behind them yet.
