# Project Indexer — Desktop app checklist

What's done and what's still open, feature by feature, for the desktop app — and for the `indexer-core` features that shipped through it (trackers, detection, persistence), since the app was their first consumer. The CLI's status is in [`../cli/checklist.md`](../cli/checklist.md). Check items off as they land; add new ones under "Open" as they come up. In `docs/`, see `accomplishments.md` for the dated story of how the checked items got done, `knowledgebase.md` for how the finished pieces actually work, and `architecture.md` for the non-feature quality backlog (invariants, detection semantics, testing, platform seams).
...
- [x] `LICENSE` (FSL-1.1-ALv2), `CLA.md`, `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, `SECURITY.md`, `ROADMAP.md`, `docs/app/USAGE.md`, issue + PR templates
...
- [x] **`ensure_project` and `ensure_project_named` disagree about binned projects.** `ensure_project_named` skips soft-deleted rows (so a binned-then-re-cloned directory is offered again by the scanner), but `ensure_project`'s early `find_by_directory` check was unfiltered and returned the binned project. Fixed 2026-09-14 with the one-line `!p.is_deleted` filter, as CLI Foundations work; a regression test is still open in `../cli/checklist.md`. Found by the 0.3.0 whole-branch review.
...
- [ ] **The command-line tool** — tracked in [`../cli/checklist.md`](../cli/checklist.md).
- [ ] **Updater fast-follows** (see `architecture.md` "Cross-app & updates"): `tauri-plugin-updater` wiring + `core::updates::latest_stable` (considering only `v*` tags), a dismissible GUI release-notification chip, tag→signed-bundle→GitHub-Release CI. The CLI `self-update` and GUI-downloads-the-CLI items were dropped on 2026-09-14 in favour of package managers.
...
- [ ] **The app provides `indexer` from its own binary** — a GUI-only install should still have the command, for people and AI assistants alike. The app's `main` runs the CLI when invoked under that name, before Tauri starts, and an "Install command-line tool" action puts it on `PATH`. Not a bundled second copy: `indexer` is ~1.7 MB compressed, almost all of it core and SQLite the app already ships. Brief: `../handoffs/2026-09-15-gui-provides-indexer.md`; tracked with its sub-tasks in `../cli/checklist.md` → *Later — packaging*.
