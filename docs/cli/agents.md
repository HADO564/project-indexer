...
| `indexer list --view <set> --json` | the same array drawn from a different set: `all` (the default), `favorites`, or `binned`. The view picks the source, so `--tracker` and a query still narrow whatever it returns. **A binned project appears only under `binned`** — favouriting is stored independently of binning, so a favourited project that is binned drops out of `favorites` and comes back when restored |
| `indexer list --sort <field> [--reverse] --json` | the same array, ordered by `name` (A to Z) or `last-opened` (most recent first). `--reverse` flips whichever field was chosen — it is not a synonym for descending, because the two fields have opposite natural directions. Default `name` |
...
| `indexer edit <query> --description <text> --json` | the [project](#a-project) as saved — every field, not only what changed. `--description ""` clears the description. At least one field flag is required: a bare `edit` is a usage error, exit 2. More field flags join this command as they land |
