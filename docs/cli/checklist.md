...
- [ ] `show`, `add`, `open`, `untrack`
  ...
  - [ ] *Next branch:* per-tracker columns and details. Several matches: the TRACKERS column becomes that tracker's own columns (git: BRANCH, CHANGES; Unreal: ENGINE) — `project_table` is fixed at four columns today, so this is where the work is. One match: the detail view gets a section under the tracker's name (git: branch, uncommitted changes, remote). The per-tracker text matches every `Tracker` variant with no `_` arm, so a new detector fails the build until it has one
