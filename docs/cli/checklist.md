...
- [ ] The write commands — favourite, description, tags, properties — and the bin (restore, purge). Briefed in [`../handoffs/2026-09-23-cli-write-commands.md`](../handoffs/2026-09-23-cli-write-commands.md). Two traps recorded there: `UpdateProject.tags` and `.properties` **replace** rather than merge, so adding one tag is a read-modify-write; and `find_one` resolves through `list`, which filters `!is_deleted`, so `restore` and `purge` cannot find the very projects they act on until they resolve against `list_deleted`
- [ ] Groups
