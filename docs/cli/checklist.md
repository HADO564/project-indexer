...
- [ ] `show`, `add`, `open`, `untrack`
  ...
  - [x] An exact name wins before the name search: `show api` picks the project named `api` even when `api-gateway` also exists
  ...
  - [ ] *Later, not this branch:* ids for scripts and copy-paste — `show <full id>` and an 8-character short id, printed in the several-matches list. Nobody types an id from memory, so it waits until the list shows one (`show <id>` stopped working when `show` switched to name matching)
